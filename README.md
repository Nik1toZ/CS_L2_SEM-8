...
Курс: Киберфизические системы. Вариант: задание на тройку.

## Задача

Развернуть локальный HTTP-сервер Ollama с моделью Qwen2.5:0.5B, отправить на него 10 произвольных запросов через Python-скрипт (библиотека `requests`) и сформировать отчёт инференса в виде таблицы из двух колонок: запрос к LLM и соответствующий ответ модели.

## Набор запросов

10 запросов, разделённых на две тематические группы:

-   5 вопросов по футболу
-   5 отвлечённых вопросов

Пары №2/№3 и №8/№9 - один и тот же запрос на русском и английском, чтобы сравнить качество ответов на разных языках.
...
Все 10 запросов отработаны, сервер отвечал стабильно. Но качество ответов Qwen2.5:0.5B низкое. Модель часто выдумывает факты, а даже верные ответы приходят с лишним текстом.

На английском модель отвечает лучше, чем на русском: тот же вопрос про пенальти на русском превратился в пять пунктов с иностранными словами, а на английском дал короткий и связный ответ. Инструкции модель тоже держит плохо - «ответь числом» даёт целое предложение, «ответь двумя названиями» заканчивается отказом. Простые вопросы (сколько ног) при этом решаются верно. Это типичные слабости моделей 0.5B: плохая память на факты, перекос в сторону английского, слабое следование формату. В реальных задачах её в чистом виде не использовать. Нужно дообучение, подключение внешних знаний или более крупная модель.

## Запуск

Ноутбук рассчитан на запуск в Google Colab.

1.  Открыть `LAB2.ipynb` в Google Colab.
2.  Запустить все ячейки: `Runtime - Run all`.

Ячейки ноутбука последовательно: устанавливают Ollama, поднимают сервер в фоне через `subprocess.Popen`, скачивают модель `qwen2.5:0.5b`, выполняют sanity-check через curl, определяют функции инференса с документацией, прогоняют 10 запросов и формируют отчёт, отображают отчёт в ноутбуке и скачивают его.
